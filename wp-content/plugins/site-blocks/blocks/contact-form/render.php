...
<div class="sg-contact-form-card" aria-labelledby="sg-contact-form-heading">
	...
	<?php if ( $status_message ) : ?>
		<div
			class="sg-contact-form-status <?php echo $is_success ? 'sg-contact-form-status--success' : 'sg-contact-form-status--error'; ?>"
			role="status"
			aria-live="polite"
		>
	...
	<form
		...
		class="sg-contact-form<?php echo $is_success ? ' sg-contact-form--success' : ''; ?><?php echo $is_error ? ' sg-contact-form--error' : ''; ?>"
		method="post"
	...
	>
		...
		<div class="sg-contact-form__field">
			<label class="sg-contact-form__label" for="contact_name">
				<?php esc_html_e( 'Name', 'site-blocks' ); ?>
				<span class="sg-contact-form__required" aria-hidden="true">*</span>
			</label>
			<input
				type="text"
				id="contact_name"
				name="contact_name"
				class="sg-contact-form__input"
				required
				autocomplete="name"
				aria-required="true"
				placeholder="<?php esc_attr_e( 'Your name', 'site-blocks' ); ?>"
			>
			<p class="sg-contact-form__helper" aria-live="polite"></p>
		</div>

		<div class="sg-contact-form__field">
			<label class="sg-contact-form__label" for="contact_email">
		...
		<div class="sg-contact-form__field">
			<label class="sg-contact-form__label" for="contact_subject">
		...
		<div class="sg-contact-form__field">
			<label class="sg-contact-form__label" for="contact_message">
			...
			<textarea
				...
				class="sg-contact-form__input sg-contact-form__textarea"
				required
				rows="6"
				aria-required="true"
		...
		<div class="sg-contact-form__actions">
			<button type="submit" class="sg-btn sg-btn--primary sg-contact-form__submit">
				<span class="sg-contact-form__submit-text"><?php echo esc_html( $submit_label ); ?></span>
				<span class="sg-contact-form__submit-spinner" aria-hidden="true"></span>
			</button>
		</div>
	</form>
</div>
...
(function () {
	...
	form.addEventListener('submit', function () {
		...
		form.querySelector('.sg-contact-form__submit').disabled = true;
	});

	var fields = form.querySelectorAll('.sg-contact-form__input');
	...
	function validateField(field) {
		...
		var helper = field.closest('.sg-contact-form__field').querySelector('.sg-contact-form__helper');
		...
		if (!valid) {
			...
			helper.classList.add('sg-contact-form__helper--error');
		} else {
			field.removeAttribute('aria-invalid');
			field.removeAttribute('data-state');
			helper.textContent = '';
			helper.classList.remove('sg-contact-form__helper--error');
		}
	}
})();
</script>
